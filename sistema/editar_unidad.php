...
require_once "includes/verifica_sesion.php";
    ...
    if(isset($_POST['btn-signup'])){
        ...
        if(empty($descripcion)){
            ...
            $query_update = mysqli_query($conection,"UPDATE unidades SET abreviatura_unidad='$unidad', descripcion='$descripcion' WHERE id_unidad='$id_unidad'");
            if ($query_update){
            ...
            ?>
				<form action="" method="post" name="miForm">
					<div class="divisor_resp">
						...
						<input type="text" name="unidad" id="unidad" value="<?php echo $unidad ?>" maxlength="25" required title="Tamaño mínimo: 2. Tamaño máximo: 25" autofocus onchange="javascript:this.value=this.value.toUpperCase();" <?php if(isset($code) && $code == 1){ echo "autofocus"; } ?> />
					</div>
					<div class="divisor_resp">
						<label for="descripcion">Descripción</label>
						<input type="text" name="descripcion" id="descripcion" value="<?php echo $descripcion ?>" maxlength="100" required title="Tamaño mínimo: 2. Tamaño máximo: 100" onchange="javascript:this.value=this.value.toUpperCase();" <?php if(isset($code) && $code == 2){ echo "autofocus"; } ?> />
					</div>
					<div class="divisor_resp"></div>
					<input type="hidden" name="id_unidad" id="id_unidad" value="<?php echo $id_unidad ?>" />
					<button type="submit" name="btn-signup" class="btn_save"><i class="fas fa-plus-circle"></i> Guardar Cambios</button>
				</form>
		</div>
